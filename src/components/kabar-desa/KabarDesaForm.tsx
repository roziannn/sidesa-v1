...
export default function KabarDesaForm({ initialData }: { initialData?: any }) {
  ...
  // Sinkronisasi jika initialData ada (Mode Edit)
  useEffect(() => {
  if (initialData) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (files.length > 0 && files[0].file) {
        ...
        const { error: uploadError } = await supabaseClient.storage.from('kabar-desa').upload(fileName, file);
        if (uploadError) throw uploadError;
        thumbnailUrl = supabaseClient.storage.from('kabar-desa').getPublicUrl(fileName).data.publicUrl;
      }

      const slug = judul.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      const payload = {
        judul, slug, konten, kategori, status,
      ...
      if (initialData) {
        const { error } = await supabaseClient.from('kabar_desa').update(payload).eq('id', initialData.id);
        if (error) throw error;
        showToast('success', 'Berhasil', 'Kabar diperbarui.');
      } else {
        const { error } = await supabaseClient.from('kabar_desa').insert([payload]);
        if (error) throw error;
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
      <FileUpload label="Thumbnail Berita" files={files} onChange={setFiles} maxFiles={1} accept="image/*" />
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 py-2">
          <input type="checkbox" checked={isDisplayed} onChange={(e) => setIsDisplayed(e.target.checked)} className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500" />
