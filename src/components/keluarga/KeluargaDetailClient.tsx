...
export default function KeluargaDetailClient({ keluarga }: KeluargaDetailClientProps) {
  ...
  const bukaModalEditAnggota = (anggota: AnggotaKeluarga) => {
    ...
    setFormNik(anggota.nik);
    setFormHubungan(anggota.hubungan);
    setFormTglLahir(anggota.tgl_lahir);
    setFormJk(anggota.jenis_kelamin);
    setFormErrorNik("");
    setShowSuksesiWarning(false);
    setIsFormModalOpen(true);
  };
  ...
  const eksekusiSimpanData = async () => {
    ...
    try {
      ...
      if (formHubungan === "Kepala Keluarga" && sudahAdaKepala && formId !== kepalaSaatIni?.id) {
        ...
        await supabaseClient.from("keluarga").update({ nama_kepala: formNama }).eq("id", keluarga.id);
      }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          ...
          <div className="relative w-full max-w-md bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden animate-scale-up">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
              <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wide">{formMode === "tambah" ? "➕ Tambah Anggota Baru" : "📝 Edit Biodata Anggota"}</h4>
              <button onClick={() => setIsFormModalOpen(false)} disabled={isFormSubmitting} className="text-slate-400 hover:text-slate-600">
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl flex items-start gap-3 text-amber-800">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-amber-700 leading-relaxed">
                      KK ini sudah memiliki kepala keluarga aktif bernama <span className="font-bold underline">{kepalaSaatIni?.nama}</span>. Apakah Anda yakin ingin mengganti jabatan Kepala Keluarga ke warga baru ini?
                    </p>
                  </div>
                </div>
                <p className="text-xs text-slate-400 italic bg-slate-50 p-2 rounded-lg border">Note: Jabatan {kepalaSaatIni?.nama} otomatis akan diturunkan menjadi Lainnya dan nama kepala pada arsip utama akan diperbarui.</p>
                <div className="flex justify-end gap-2 pt-2">
                  <button type="button" onClick={() => setShowSuksesiWarning(false)} className="px-3 py-1.5 border bg-white rounded-lg text-xs font-semibold text-slate-600 hover:bg-slate-50">
                    Kembali
                  </button>
                  <button type="button" onClick={eksekusiSimpanData} className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-xs font-bold shadow-sm">
                    Ya, Ganti Jabatan
                  </button>
                </div>
              </div>
              ...
              <form onSubmit={handlePreSubmitCheck} className="p-5 space-y-4 text-sm">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-slate-300 rounded-lg p-2 text-slate-800 bg-white text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">
                    Nomor NIK (16 Digit) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    maxLength={16}
                    ...
                    className={`w-full border rounded-lg p-2 font-mono text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 ${formErrorNik ? "border-red-500 bg-red-50/50" : "border-slate-300 bg-white"}`}
                  />
                ...
                <div>
                  ...
                  <select value={formHubungan} onChange={(e) => setFormHubungan(e.target.value)} className="w-full border border-slate-300 bg-white text-sm rounded-lg p-2 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                    {["Kepala Keluarga", "Suami", "Istri", "Anak", "Orang Tua", "Mertua", "Cucu", "Saudara", "Lainnya"].map((hub) => (
                ...
                {/* BAGIAN DIBAWAH INI SEKARANG BERURUTAN KE BAWAH (TIDAK GRID KANAN-KIRI LAGI) */}
                <div>
                  <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">
                  ...
                  <input
                    ...
                    className="w-full border border-slate-300 bg-white text-sm rounded-lg p-2 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 pt-1">
                    <label className="flex items-center gap-2 cursor-pointer text-slate-700 font-semibold text-xs select-none">
                      ...
                      <span className={`inline-flex items-center gap-1 p-1 rounded-md ${formJk === "L" ? "bg-blue-50 text-blue-700" : "text-slate-400"}`}>
                        <Smile className="w-4 h-4 stroke-[2.5]" /> Laki-laki
                      </span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer text-slate-700 font-semibold text-xs select-none">
                      <input type="radio" name="jk" checked={formJk === "P"} onChange={() => setFormJk("P")} className="text-pink-600 w-4 h-4" />
                      <span className={`inline-flex items-center gap-1 p-1 rounded-md ${formJk === "P" ? "bg-pink-50 text-pink-700" : "text-slate-400"}`}>
                        <Smile className="w-4 h-4 stroke-[2.5]" /> Perempuan
                      </span>
                    </label>
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-3 border-t border-slate-100 mt-5">
                  <button type="button" onClick={() => setIsFormModalOpen(false)} disabled={isFormSubmitting} className="px-4 py-2 border border-slate-300 bg-white text-slate-700 font-semibold rounded-lg hover:bg-slate-50">
                    Batal
                  </button>
                  <button type="submit" disabled={isFormSubmitting} className="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white font-semibold rounded-lg shadow-sm">
